...
Boolean mp4_read_avcC_box(void *arg, FILE *mp4_file)
{
    ...
    avcc_box->sps = (struct SPS *)malloc(sizeof(struct SPS) * avcc_box->numOfSequenceParameterSet);
    ...
    avcc_box->pps = (struct PPS *)malloc(sizeof(struct PPS) * avcc_box->numOfPictureParameterSets);
...
int main()
{
    ...
    if ((mp4_file = fopen("yueyu.mp4", "rb+")) == NULL)
        err_exit("fopen");
